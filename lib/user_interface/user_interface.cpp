...
void displayInit() {
    ...
    // Vùng hiển thị thanh loading - nằm sát dưới cùng
    int barHeight = 10; // Giảm độ cao thanh loading
    int barY = height - barHeight - 5; // Chỉ nhô lên 5px từ đáy
    int barWidth = width - 20; // Rộng hơn một chút 
    int barX = (width - barWidth) / 2;
    
    // Viền ngoài thanh loading - màu vàng chanh với hiệu ứng mỏng
    tft.drawRoundRect(
        barX, barY, 
        barWidth, barHeight, 
        barHeight/2, 
    ...
    // Không cần xóa vùng background vì sẽ vẽ trực tiếp lên nền hiện tại
    
    // ===== PHẦN 7: QUÁ TRÌNH LOADING VỚI THÔNG BÁO =====
    ...
    for (int step = 0; step < 5; step++) {
        // Không hiển thị thông báo phần trăm nữa
        ...
        for (int p = startPercent; p <= endPercent; p++) {
            ...
            // Thanh tiến trình - màu gradient từ vàng đến trắng
            ...
            // Vẽ trực tiếp thanh progress không có nền
            tft.fillRoundRect(
                barX + 2, barY + 2,
            ...
            delay(15); // Làm chậm tiến trình loading
        }
